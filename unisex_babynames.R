...
geom_line(aes(y=prop_f), size = 0.35) +
  ...
  labs(
    ...
    caption = "Source : Babynames R Packages | By : Abdoul ISSA BIDA inspired by Nathan YAU"
  ) +   
  coord_cartesian(expand = F) +
  theme_minimal(base_family = "Lato") + 
  theme(
    plot.title = element_text(family = "Lato Black", size = rel(1.8), hjust =.5, margin = margin(b = .5,unit = "cm")),
    axis.line.x = element_line(),
    axis.ticks.y = element_blank() ,
    axis.ticks.x = element_line(),
    axis.ticks.length.x = unit(0.125,"cm"),
    axis.text = element_text(color = "black"),
    strip.text = element_text(family = "Lato Black", size = rel(1.0))
  )
)
